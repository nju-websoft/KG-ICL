...
class RSPMMAddAddFunction(autograd.Function):
    ...
    def forward(ctx, edge_index, edge_type, edge_weight, relation, input):
        ...
        output = forward(edge_index, edge_type, edge_weight, relation, input, edge_index.max().item()+1)
        ctx.save_for_backward(edge_index, edge_type, edge_weight, relation, input, output)
        return output
    ...
    def backward(ctx, output_grad):
        ...
        edge_index, _, _, _, _, _ = ctx.saved_tensors
        # print('**********************************', ctx.saved_tensors[0].max().item()+1)
        weight_grad, relation_grad, input_grad = backward(*ctx.saved_tensors, output_grad, edge_index.max().item()+1)
        return None, None, weight_grad, relation_grad, input_grad
